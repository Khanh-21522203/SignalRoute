...
- Dependency-free admin health and metrics service for component health aggregation, service/dependency/lifecycle probe helpers, split liveness/readiness snapshots, production-adapter readiness policy flags, dependency health source registry, adapter health helpers, Prometheus-text metrics snapshots, transport-neutral health/metrics endpoint responses, configurable HTTP-style health/metrics response binding, runtime exposure, lifecycle-aware in-process admin request loop, runtime-owned configurable loopback-capable TCP admin socket binding, runtime-owned fallback metrics scrape socket, socket read timeout, request-size cap, structured access-log event support, and endpoint-aware socket startup failure diagnostics.
...
- Dependency-free local CI verification script, hosted fallback/protobuf/sanitizer CI workflow, production fallback Dockerfile, adapter image scaffold, protobuf adapter image target, Kafka adapter image target, Docker Compose dependency scaffold, integration harness manifest, container runtime contract, and CMake sanitizer profiles for ASan, UBSan, and TSan.
...
- Kafka package-backed verification is currently blocked with the `system` provider: Ubuntu 24.04 `librdkafka-dev` installs headers/libs but does not provide `RdKafkaConfig.cmake` or `rdkafka-config.cmake` for `find_package(RdKafka CONFIG REQUIRED)`.
...
- Runtime composition exposes configurable admin HTTP health/readiness/metrics responses through a lifecycle-aware request loop, optional config-driven TCP admin socket binding, and an opt-in runtime-owned metrics scrape socket; `/health` is liveness-focused and `/ready` carries service readiness and required Kafka/Redis/PostGIS/H3 dependency health source failures. The admin socket and metrics exporter enforce configured read timeout and max request bytes, and startup failures report endpoint details. Runtime shutdown tracks a stop reason for signal, unhealthy, fatal, requested, and destructor paths. prometheus-cpp registry integration and gRPC server binding remain pending.
...
- **Status:** Done fallback for role-specific runtime ownership, admin probes, shutdown order, stop reason tracking, and admin socket startup failure cleanup; production cross-role wiring pending.
...
- **Status:** Done fallback; optional librdkafka++ producer/consumer adapter path added behind `SR_ENABLE_REAL_KAFKA`; `adapter-kafka` Bake target added, but Ubuntu 24.04 `librdkafka-dev` lacks the required `RdKafka` CMake config package, so provider-backed compile/run verification remains blocked.
...
- **Status:** Dependency-free admin health aggregation, service/dependency/lifecycle probe helpers, split liveness/readiness snapshots, configurable production-adapter readiness policy flags, dependency health source registry, adapter health helpers, metrics snapshot service, transport-neutral endpoint handler, configurable HTTP-style health/metrics binding, runtime composition exposure, lifecycle-aware in-process admin request loop, optional runtime-owned TCP admin socket binding, opt-in runtime-owned fallback metrics scrape socket, socket timeout/size hardening, structured socket access-log events, endpoint-aware socket startup diagnostics, process-level probe registration, and gated gRPC adapter skeleton added; prometheus-cpp registry integration and real gRPC server binding pending.
- **Goal:** Complete metrics, health, readiness, and admin APIs.
- **Items:** prometheus-cpp registry integration, health aggregation, component status events, admin service.
- **Depends on:** A1/A2, C2.
- **Verification:** metrics/admin tests.

### Agent Task O1: Packaging And CI
- **Ownership:** Docker/CI/build docs.
- **Status:** Local fallback/protobuf verification script, hosted fallback/protobuf/sanitizer CI workflow, manual dependency service scaffold, manual adapter image scaffold job, manual protobuf adapter image job, manual Kafka adapter image job, manual integration harness job, production fallback Dockerfile, adapter image scaffold, protobuf adapter image target, Kafka adapter image target, Docker Compose dependency scaffold, integration harness manifest, container runtime contract, and sanitizer CMake profiles added; Kafka image target is blocked on a `RdKafka` CMake config provider, and real dependency-backed integration jobs remain pending.
...
Current package boundary: `adapter-kafka` installs Ubuntu `librdkafka-dev`/`librdkafka++1`, but `SR_ENABLE_REAL_KAFKA=ON` fails at CMake configure because that package does not provide `RdKafkaConfig.cmake` or `rdkafka-config.cmake`. The next Kafka production batch must choose a provider strategy before broker-backed tests can be added.
...
- Implement metrics reporter or direct Prometheus exposer.
  - Status: opt-in dependency-free runtime scrape socket serves current Prometheus text snapshots at `observability.metrics_path`; package-backed prometheus-cpp registry integration remains pending.
- Implement admin health service.
  - Status: dependency-free health aggregation, service/dependency probe helpers, split liveness/readiness handling, configurable production-adapter readiness policy flags, dependency health source registry, adapter health helpers, metrics snapshot service, transport-neutral endpoint handler, configurable HTTP-style response binding, runtime exposure, lifecycle-aware in-process admin request loop, optional runtime-owned TCP admin socket binding, socket timeout/size hardening, structured socket access-log events, and endpoint-aware socket startup diagnostics added; gRPC binding pending.
...
- Add graceful shutdown for all services.
  - Status: runtime stops services in reverse order and records stop reason for requested, signal, unhealthy, fatal, and destructor-triggered shutdown paths.
- Add readiness/liveness semantics.
  - Status: common lifecycle snapshots, service readiness APIs, process-level admin registration, split `/health` vs `/ready` semantics, config-driven production dependency readiness requirements, and dependency health source injection added.
...
- `/metrics` or equivalent exposes useful counters/gauges/histograms.
  - Status: runtime-owned fallback metrics exporter serves counters, gauges, and observation count/sum snapshots when `observability.metrics_exporter_enabled = true`.
...
- Add Docker Compose for local dependencies:
  ...
  - Redis
  - TimescaleDB/PostGIS
  - Status: `compose.yml` adds Redis, PostGIS, and Redpanda with healthchecks and named volumes. SignalRoute service is profile-gated and still runs fallback runtime by default.
...
- Add adapter image scaffold.
  - Status: `Dockerfile.adapters` and `docker-bake.hcl` provide a repeatable fallback-safe image path, a protobuf-only package-backed image path, and a Kafka package-backed image target that currently documents the missing `RdKafka` CMake config blocker. Real adapter switches stay off by default.
- Add CI pipeline:
  - format check
  - build
  - unit tests
