...
| Redis | `SR_ENABLE_REAL_REDIS` | `find_package(Hiredis REQUIRED)` through `cmake/FindHiredis.cmake` | `hiredis::hiredis`, `hiredis` | `libhiredis-dev`; Ubuntu 24.04's packaged `HiredisConfig.cmake` sets variables but no imported target, so the repository finder bridges headers/libs directly | `libhiredis1.1.0` | `integration:state`, `integration:nearby`, `integration:matching` |
...
| H3 | `SR_ENABLE_REAL_H3` | `find_package(h3 REQUIRED)` through `cmake/Findh3.cmake` | `h3::h3`, `h3` | `libh3-dev`; Ubuntu 24.04 ships a faulty `h3Config.cmake` that references missing `/usr/lib/bin/h3`, so the repository finder bridges headers/libs directly | `libh3-1` | `integration:nearby`, `integration:geofence` |
...
| 63 | H3 | `libh3-dev` and `libh3-1` install in package-backed images; Ubuntu 24.04's packaged `h3Config.cmake` is unusable because `h3::h3_bin` points at missing `/usr/lib/bin/h3`, so `cmake/Findh3.cmake` bridges `h3api.h`/`libh3` to `h3::h3`; `adapter-h3` and `adapter-h3-test` build, `test_h3_index` passes, and the runtime links `libh3.so.1` | Keep H3 image target available; compose real nearby/geofence integrations after Redis/PostGIS providers are verified |
| 64 | Redis | `libhiredis-dev` and `libhiredis1.1.0` install in package-backed images; `cmake/FindHiredis.cmake` bridges `hiredis/hiredis.h`/`libhiredis` to `hiredis::hiredis`; `adapter-redis` and `adapter-redis-test` build, `test_redis_client` passes against a real Redis container, and the runtime links `libhiredis.so.1.1.0` | Keep Redis image target available; compose state/nearby/matching integrations after PostGIS and feature harness coverage are ready |
...
## Current Boundary
`adapter-protobuf` is the first working package-backed image path and enables only generated protobuf messages. `adapter-kafka` builds with Ubuntu 24.04 `librdkafka-dev` through the explicit `FindRdKafka.cmake` bridge and enables only `SR_ENABLE_REAL_KAFKA=ON`. `adapter-h3` builds with Ubuntu 24.04 `libh3-dev` through the explicit `Findh3.cmake` bridge and enables only `SR_ENABLE_REAL_H3=ON`; `adapter-h3-test` verifies the H3 package path with `test_h3_index`. `adapter-redis` builds with Ubuntu 24.04 `libhiredis-dev` through the explicit `FindHiredis.cmake` bridge and enables only `SR_ENABLE_REAL_REDIS=ON`; `adapter-redis-test` verifies the Redis package path with `test_redis_client` against a real Redis container. `integration-ingestion` builds a Kafka+protobuf test image and the Redpanda-backed `integration:ingestion` test now covers real produce/consume, processor state/history writes, and committed offsets. PostGIS, gRPC, Prometheus, and toml++ remain off by default.
