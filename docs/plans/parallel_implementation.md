...
- Admin health/metrics now has a dependency-free service boundary for component health aggregation, service/dependency/lifecycle probe helpers, readiness snapshots, and Prometheus-text metric snapshots. Real gRPC/HTTP endpoint binding remains pending.
- Runtime startup now flows through a dependency-free `RuntimeApplication` composition boundary that owns role-selected services, registers process-level admin lifecycle probes, aggregates readiness/health, and stops services in reverse order.
...
Continue with production config/runtime hardening: validate runtime role configuration, load dependency endpoints from `config/signalroute.toml`, and make `RuntimeApplication` report explicit startup errors instead of relying on default fallback values. Only run package-backed adapter/endpoint checks after the relevant dependency path changes or after dependencies are installed. Keep CSV fallback decoding until durable Kafka integration tests pass. gRPC service stubs remain gated behind `SR_ENABLE_GRPC`.
