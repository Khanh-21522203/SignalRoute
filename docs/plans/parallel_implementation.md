...
- Hosted CI plus local sanitizer commands are tracked in `docs/plans/ci_and_sanitizers.md`; use `scripts/verify-local.sh` for dependency-free fallback/protobuf verification. Manual dependency service, adapter image scaffold, and protobuf adapter image jobs exist for Redis/PostGIS/Redpanda provisioning, fallback-safe image checks, and protobuf package-backed image checks, but they do not enable real adapter switches.
- Production fallback container packaging, adapter image scaffolding, protobuf adapter image packaging, and Docker Compose dependency scaffolding are tracked in `docs/plans/container_packaging.md`; real adapter wiring remains a separate future scope.
...
Continue with broker-backed Kafka verification first: add an `adapter-kafka` package-backed image target, then add `integration:ingestion` tests that consume the manual Redpanda scaffold. The protobuf image target is available as the first package-backed image path and still keeps real Kafka, Redis, PostGIS, H3, gRPC, Prometheus, and toml++ switches off. If gRPC packages are installed, compile `SR_ENABLE_GRPC=ON`, fix any package-backed adapter compile issues, then add a minimal server composition object that registers admin/gateway/query gRPC services by role. Keep CSV fallback decoding until durable Kafka integration tests pass.
