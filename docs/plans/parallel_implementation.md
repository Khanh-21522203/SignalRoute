...
- Admin health/metrics now has a dependency-free service boundary for component health aggregation, service/dependency/lifecycle probe helpers, readiness snapshots, Prometheus-text metric snapshots, and transport-neutral health/metrics endpoint responses. Real gRPC/HTTP endpoint binding remains pending.
...
Continue with transport binding preparation: add generated gRPC service adapter skeletons for admin/gateway/query only when `SR_ENABLE_GRPC=ON`, reusing existing transport-neutral handlers and leaving fallback builds dependency-free. Only run package-backed adapter/endpoint checks after the relevant dependency path changes or after dependencies are installed. Keep CSV fallback decoding until durable Kafka integration tests pass.
