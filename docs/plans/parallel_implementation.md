...
Continue with process-level admin probe registration at startup/composition and graceful shutdown orchestration tests. Only run package-backed adapter/endpoint checks after the relevant dependency path changes or after dependencies are installed. Keep CSV fallback decoding until durable Kafka integration tests pass. gRPC service stubs remain gated behind `SR_ENABLE_GRPC`.
