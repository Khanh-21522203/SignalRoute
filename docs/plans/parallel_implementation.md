...
1. Resolve the Kafka provider gap by adding a deliberate `FindRdKafka.cmake`, vcpkg/Conan provider, or source/package overlay that supplies the `RdKafka` CMake target
2. Broker-backed Kafka compile/integration verification once RdKafka is installed
...
Resolve the Kafka CMake provider gap first. `adapter-kafka` proves that Ubuntu 24.04 `librdkafka-dev` installs headers/libs but does not provide `RdKafkaConfig.cmake`; the next implementation should either add a deliberate `FindRdKafka.cmake` bridge over pkg-config/imported targets or switch the Kafka adapter image to a provider that supplies `RdKafka::rdkafka++`. After that compiles, add `integration:ingestion` tests that consume the manual Redpanda scaffold. Keep CSV fallback decoding until durable Kafka integration tests pass.
