...
- Hosted CI plus local sanitizer commands are tracked in `docs/plans/ci_and_sanitizers.md`; use `scripts/verify-local.sh` for dependency-free fallback/protobuf verification. Manual dependency service, adapter image scaffold, protobuf adapter image, Kafka adapter image, H3 adapter image, Redis adapter image, and ingestion integration jobs exist for Redis/PostGIS/Redpanda provisioning, fallback-safe image checks, package-backed image checks, and Redpanda-backed ingestion checks, but they do not affect default CI.
- Production fallback container packaging, adapter image scaffolding, protobuf adapter image packaging, Kafka adapter image packaging, H3 adapter image packaging, Redis adapter image packaging, ingestion integration image packaging, and Docker Compose dependency scaffolding are tracked in `docs/plans/container_packaging.md`; real endpoint and cross-service feature wiring remains a separate future scope.
...
- Use the locked CTest labels from `tests/integration/README.md`: `integration:ingestion`, `integration:state`, `integration:trip-history`, `integration:nearby`, `integration:geofence`, and `integration:matching`.
...
1. Real-PostGIS compile/integration verification once PostgreSQL/libpq and a PostGIS database are available
2. State/nearby/matching integration that composes Redis with feature dependencies after PostGIS and harness coverage are ready
3. Nearby/geofence integration that composes Redis/PostGIS/H3 after the storage providers are verified
...
Run real-PostGIS package-backed verification next. Redis now uses the Ubuntu `libhiredis-dev` package path directly, H3 is package-backed, and Kafka ingestion has Redpanda-backed `integration:ingestion` coverage; keep CSV fallback decoding until the remaining Kafka boundaries (geofence, matching, DLQ/workers) have durable protobuf-backed integration tests.
