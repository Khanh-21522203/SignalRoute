...
1. Real-Redis compile/integration verification once hiredis/redis++ are installed or a provider overlay is chosen
2. Real-PostGIS compile/integration verification once PostgreSQL/libpq and a PostGIS database are available
...
Run real-Redis provider verification next. Ubuntu 24.04 provides hiredis but no standard redis-plus-plus archive package in the checked package set, so the next batch should either lock a source/package overlay for redis-plus-plus or deliberately pivot to PostGIS package-backed verification first. Kafka ingestion now has Redpanda-backed `integration:ingestion` coverage; keep CSV fallback decoding until the remaining Kafka boundaries (geofence, matching, DLQ/workers) have durable protobuf-backed integration tests.
