...
Continue with runtime operations polish: add structured startup/shutdown log events and expose runtime/admin health output in a transport-neutral handler that can later back HTTP/gRPC health endpoints. Only run package-backed adapter/endpoint checks after the relevant dependency path changes or after dependencies are installed. Keep CSV fallback decoding until durable Kafka integration tests pass. gRPC service stubs remain gated behind `SR_ENABLE_GRPC`.
