...
| `SR_ENABLE_REAL_REDIS` | `OFF` | `hiredis::hiredis` or `hiredis` | Real state, H3 index, fence state, reservations |
...
- Ubuntu 24.04 `libhiredis-dev` ships a `HiredisConfig.cmake` that sets variables but no imported target; `cmake/FindHiredis.cmake` is the locked system-provider bridge for `SR_ENABLE_REAL_REDIS`.
...
- Real Redis adapter code is hidden behind the existing wrapper API and pimpl storage; public headers do not expose hiredis types.
...
5. Install/provide PostgreSQL/libpq headers and libraries, then run real-PostGIS compile/integration verification for `SR_ENABLE_REAL_POSTGIS`; the adapter path is present behind `PostgresClient`.
6. Add state/nearby/matching integration coverage that composes Redis with the relevant feature dependencies after PostGIS and feature harness coverage are ready.
...
| Redis adapter image | `docker buildx bake adapter-redis adapter-redis-test` or GitHub Actions `workflow_dispatch` with `run_adapter_redis_image=true` | Builds and smoke-tests a Redis-enabled adapter image through `cmake/FindHiredis.cmake`, then runs `test_redis_client` against a real Redis service; Kafka/protobuf/gRPC/PostGIS/H3/Prometheus/toml++ stay off |
