...
## Purpose
Batch 17 established the build contract for production dependencies without replacing the fallback runtime. Batch 18 added domain-to-wire conversion contracts. Batch 19 adds protobuf-only generated builds and keeps gRPC stubs optional because local gRPC packages may not be installed. Batch 47 keeps that contract while adding dependency-free local CI and sanitizer build profiles. Batch 57 locks package-backed adapter naming, package candidates, CI promotion rules, and feature-grouped integration labels in `docs/plans/package_strategy_lock.md`. Batch 59 verifies that Ubuntu 24.04 `librdkafka-dev` does not provide the required `RdKafka` CMake config package.
...
| Kafka adapter image target | `docker buildx bake --print adapter-kafka`; `docker buildx bake adapter-kafka` currently fails at CMake configure with Ubuntu `librdkafka-dev` | Bake target is defined, but `find_package(RdKafka CONFIG REQUIRED)` is blocked because Ubuntu 24.04 does not provide `RdKafkaConfig.cmake`/`rdkafka-config.cmake` |
